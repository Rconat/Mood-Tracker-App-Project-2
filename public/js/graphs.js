...
$(document).ready(() => {
  ...
  const labelArray = [];
  const dataArray = [];
  const labelGroupArray = [];
  const dataWithOthers = [];
  const dataNotWithOthers = [];
  const dataEatenToday = [];
  const dataNotEatenToday = [];
  const dataTakenMeds = [];
  const dataNotTakenMeds = [];
  ...
  function handleWithOthers_Click() {
    AddDataSet(
      datasetWithOthers,
      datasetAlone,
      "Mood Ratings: With Others (pets included)"
    );
  }
  function handleEatenToday_Click() {
    AddDataSet(
      datasetEatenToday,
      datasetNotEaten,
      "Mood Ratings: When Eaten Today (not just coffee)"
    );
  }
  function handleMeds_Click() {
    AddDataSet(
      datasetTakenMeds,
      datasetNoMeds,
      "Mood Ratings: Taken Medications"
    );
  }
  ...
  function PlotGraph(labels, data) {
    ...
    // eslint-disabled as this is needed by Charts.js npm
